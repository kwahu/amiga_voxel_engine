...
voxel_draw: 
		...
		divu.w	#160,d2					;Divides the unsigned destination operand
		not.w	d2						;Calculates the ones complement of the destination operand
		addi.w	#94,d2					;Adds the immediate data to the destination operand
		add.w	d2,d2				;Adds the source operand to the destination operand using binary addition
		
		lea		voxel_scrambletab+VOXEL_WIDTH/2*2,a4 ;load address into address register
		adda.w	d6,a4							;Adds the source operand to the destination address register
		lea		voxel_blitbuf,a5
		move.l	#voxel_offsetsend,d6			;move to d6
		
		move.w	#VOXEL_WIDTH/2-7,d7
.l0:
		tst.w	(a5)							;Compares the operand with zero
...
;5 - minterm
 MACRO<VOXEL_C2PBLIT>
		...
		move.w  #VOXEL_PLANESIZE/4*64+1,(a6)+	;BLTSIZE   ;64000 + 1
...
voxel_c2p:
		lea		voxel_blitbuf,a6					;ustaw adres
		
		move.l	voxel_backbuffers+8,a0				;ustaw bufor do rysowania
		move.l	voxel_screens+0,a1					; ustaw bufor
		adda.w	#VOXEL_PLANESIZE/1*2,a0				; (160 * 100) /4 = 4000 -> +8000
		adda.w	#VOXEL_PLANESIZE/4*2,a1				; + 2000
