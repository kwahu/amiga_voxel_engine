...
UBYTE ProcessWord2ST(UBYTE rounds, UBYTE sx, UBYTE sy, UWORD *_tz, UWORD *tzz, UBYTE px, UBYTE py,UBYTE ph,
UWORD *address1, UWORD *address2, 
WORD (*rayCastX)[TERRAINDEPTH], WORD (*rayCastY)[TERRAINDEPTH], UWORD (*map)[256])
{
	UWORD mapValue;
...
UBYTE ProcessWord3ST(UBYTE rounds, UBYTE sx, UBYTE sy, UWORD *_tz, UWORD *tzz, UBYTE px, UBYTE py,UBYTE ph,
UWORD *address1, UWORD *address2, 
WORD (*rayCastX)[TERRAINDEPTH], WORD (*rayCastY)[TERRAINDEPTH], UWORD (*map)[256])
{
	UWORD mapValue;
...
void ProcessRayCastsST(WORD (*rayCastX)[TERRAINDEPTH], WORD (*rayCastY)[TERRAINDEPTH], UWORD (*map)[256],
	...
	UBYTE ySize, BYTE xOffset)
{
	...
	for(iVert=0;iVert<xCycles;iVert++)
	{
		...
		while(sy < ySize)
		{
			 if(tz < threshold1)			
			 {
				tz = ProcessWord1ST(1,sx,sy,&tz,tzz,px,py,ph,&address1,&address2,rayCastX, rayCastY, map);
			 ...
			 {
				tz = ProcessWord2ST(2,sx,sy,&tz,tzz,px,py,ph,&address1,&address2,rayCastX, rayCastY, map);
			 ...
			 {
				tz = ProcessWord3ST(3,sx,sy,&tz,tzz,px,py,ph,&address1,&address2,rayCastX, rayCastY, map);
